...
class MessageReponse$Type extends MessageType$<MessageReponse> {
    constructor() {
        super("MessageReponse", [
            ...
            { no: 7, name: "type", kind: "enum", T: () => ["MessageType", MessageType] }
        ]);
    }
    ...
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: MessageReponse): MessageReponse {
        let message = target ?? this.create(), end = reader.pos + length;
        while (reader.pos < end) {
            let [fieldNo, wireType] = reader.tag();
            switch (fieldNo) {
    ...
    internalBinaryWrite(message: MessageReponse, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        /* string id = 1; */
        if (message.id !== "")
            writer.tag(1, WireType.LengthDelimited).string(message.id);
        /* string sender = 2; */
...
class MessageRequest$Type extends MessageType$<MessageRequest> {
    constructor() {
        super("MessageRequest", [
            ...
            { no: 4, name: "type", kind: "enum", T: () => ["MessageType", MessageType] }
        ]);
    }
